...
class FinancialModel extends CI_Model {
	...
	*/
	public function getcustomerlist($filter_data){
		...
		foreach($customerListArr as $key => $val)
		{
			...
			// echo '<br/>'.$val['id'].' === '.count($cust_invoices);
			...
			{*/	
				foreach ($cust_invoices as $key1 => $value1)
				{
					...
					$total_billed += $value1['total'];

					$result[$cnt]['total'] += $value1['total'];
					
					$pay_details = $this->getinvoicepayment($value1['invoiceno'], $val['id'], $total_billed);
					
					if(!empty($pay_details)){
						$result[$cnt]['total_credited'] = $pay_details['total_credited'];
			        	$result[$cnt]['total_tds'] = $pay_details['total_tds'];
					}

	        		$result[$cnt]['total_receivable'] = $value1['total'] - $pay_details['total_credited'] - $pay_details['total_tds'];
	...
	{	
		...
		$this->db->select('COUNT(i.id) as total_invoices, b.name as billing_entity_name, b.id as billing_entity_id, i.id, i.invoiceno, i.billing_entity_id, i.total', false);
	...
	public function getinvoicepayment($invoice_no='', $account_id='', $total_billed=''){
        ...
        foreach($invoicePaymentArr as $invoicePayDet){
        	...
        	$total_tds_receivable += $invoicePayDet['tds'];
        	$total_receivable += $invoicePayDet['total_receivable'];
        }
        $total_receivable_amt = $total_billed - $total_credited - $total_tds_receivable;

        // echo 'total_billed: '.$total_billed.' == total_credited: '.$total_credited.' == total_receivable_amt: '.$total_receivable_amt.'<br/>';
	...
	*/
	...
	function get_not_yet_overdue_invoices_details($account_id){
		...
		$query = $this->db->query("select case when duedate!='' then duedate else invoicedate end as invdate, invoice.* from invoice where deleted = '0' and account_id = '".$account_id."' and number_of_days = '0'".$date_filter);
