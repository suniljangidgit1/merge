...
namespace Finnova\Core\Console;
...
class CommandManager
{
    ...
    {
        $command = ucfirst(\Finnova\Core\Utils\Util::hyphenToCamelCase($command));
        ...
        foreach ($_SERVER['argv'] as $i => $item) {
            ...
            if (strpos($item, '--') === 0 && strpos($item, '=') > 2) {
                ...
                $name = \Finnova\Core\Utils\Util::hyphenToCamelCase($name);
                $options[$name] = $value;
        ...
        $className = '\\Finnova\\Core\\Console\\Commands\\' . $command;
        ...
        if (!class_exists($className)) {
            ...
            echo $msg . "\n";
            throw new \Finnova\Core\Exceptions\Error($msg);
        }
