...
use Finnova\Core\Utils\Util,
    Finnova\Core\Exceptions\NotFound,
    Finnova\Core\Exceptions\Error;
...
class Language
{
    ...
    protected $paths = [
        'corePath' => 'application/Finnova/Resources/i18n/{language}',
        'modulePath' => 'application/Finnova/Modules/{*}/Resources/i18n/{language}',
        'customPath' => 'custom/Finnova/Custom/{*}/Resources/i18n/{language}',
    ];
    ...
    public function __construct($language = null, File\Manager $fileManager, Metadata $metadata, $useCache = false, $noCustom = false)
    {
        ...
        $this->unifier = new \Finnova\Core\Utils\File\Unifier($this->fileManager, $this->metadata);
    }
    ...
    public static function detectLanguage(\Finnova\Core\Utils\Config $config, $preferences = null)
    {
        $language = null;
        if ($preferences) {
	...
	$customDataPath = str_replace('{*}', $cacheDataPath, $this->cacheFile);
        ...
        $langCacheFile = str_replace('{language}', $language, $customDataPath);
