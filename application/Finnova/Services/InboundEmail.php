...
use \Finnova\Entities\Team;
use \Zend\Mail\Storage;

use \Finnova\Core\Exceptions\Error;
use \Finnova\Core\Exceptions\Forbidden;

class InboundEmail extends \Finnova\Services\Record
{
    private $campaignService = null;

    protected $storageClassName = '\\Finnova\\Core\\Mail\\Mail\\Storage\\Imap';
    ...
    public function fetchFromMailServer(Entity $emailAccount)
    {
        ...
        $importer = new \Finnova\Core\Mail\Importer($this->getEntityManager(), $this->getConfig(), $notificator);
        ...
        $parserClassName = '\\Finnova\\Core\\Mail\\Parsers\\' . $parserName;
        ...
        foreach ($monitoredFoldersArr as $folder) {
            ...
            foreach ($idList as $i => $id) {
                ...
                try {
                    ...
                    $message = new \Finnova\Core\Mail\MessageWrapper($storage, $id, $parser);
    ...
    {
        $className = '\\Finnova\\Custom\\Business\\Distribution\\CaseObj\\RoundRobin';
        if (!class_exists($className)) {
            $className = '\\Finnova\\Modules\\Crm\\Business\\Distribution\\CaseObj\\RoundRobin';
        }
    ...
    {
        $className = '\\Finnova\\Custom\\Business\\Distribution\\CaseObj\\LeastBusy';
        if (!class_exists($className)) {
            $className = '\\Finnova\\Modules\\Crm\\Business\\Distribution\\CaseObj\\LeastBusy';
        }
    ...
    protected function emailToCase(\Finnova\Entities\Email $email, array $params = [])
    {
        ...
        try {
            ...
            if ($replyEmailTemplateId) {
                ...
                if (empty($contact)) {
                    ...
                    $fromName = \Finnova\Services\Email::parseFromName($email->get('fromString'));
                    if (!empty($fromName)) {
    ...
    protected function getSmtpParamsFromInboundEmail(\Finnova\Entities\InboundEmail $emailAccount)
    {
        $smtpParams = [];
    ...
    public function findSharedAccountForUser(\Finnova\Entities\User $user, $emailAddress)
    {
    ...
    protected function getStorage(\Finnova\Entities\InboundEmail $emailAccount)
    {
        $imapParams = [
            'host' => $emailAccount->get('host'),
            'port' => $emailAccount->get('port'),
    ...
    public function storeSentMessage(\Finnova\Entities\InboundEmail $emailAccount, $message)
    {
    ...
    public function getSmtpParamsFromAccount(\Finnova\Entities\InboundEmail $emailAccount)
    {
        $smtpParams = [];
        $smtpParams['server'] = $emailAccount->get('smtpHost');
        if ($smtpParams['server']) {
