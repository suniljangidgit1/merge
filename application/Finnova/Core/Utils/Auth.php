...
class Auth
{
    ...
    public function __construct(\Finnova\Core\Container $container, $allowAnyAccess = false)
    {
    ...
    {
        return $this->getConfig()->get('authenticationMethod', 'Finnova');
    }

    protected function getAuthenticationImpl(string $method) : \Finnova\Core\Utils\Authentication\Base
    {
        ...
        if (!$className) {
            ...
            $className = "\\Finnova\\Custom\\Core\\Utils\\Authentication\\" . $sanitizedName;
            if (!class_exists($className)) {
                $className = "\\Finnova\\Core\\Utils\\Authentication\\" . $sanitizedName;
            }
        }
    ...
    protected function get2FAImpl(string $method) : \Finnova\Core\Utils\Authentication\TwoFA\Base
    {
        $className = $this->getMetadata()->get([
        ...
        if (!$className) {
            ...
            $className = "\\Finnova\\Custom\\Core\\Utils\\Authentication\\TwoFA\\" . $sanitizedName;
            if (!class_exists($className)) {
                $className = "\\Finnova\\Core\\Utils\\Authentication\\TwoFA\\" . $sanitizedName;
            }
        }
    ...
    public function login($username, $password = null, $authenticationMethod = null)
    {
        ...
        if (!$authenticationMethod) {
            if ($this->request->headers->get('Http-Finnova-Authorization-By-Token') === 'true') {
                $isByTokenOnly = true;
            }
        }

        $createTokenSecret = $this->request->headers->get('Finnova-Authorization-Create-Token-Secret') === 'true';
        ...
        if (!$user->isAdmin() && $this->getConfig()->get('maintenanceMode')) {
            throw new \Finnova\Core\Exceptions\ServiceUnavailable("Application is in maintenance mode.");
        }
        ...
        if (!$authToken && $this->getConfig()->get('auth2FA')) {
            ...
            if ($twoFAMethod) {
                ...
                $twoFACode = $this->request->headers->get('Finnova-Authorization-Code');
        ...
        if (!$secondStepRequired && $this->request->headers->get('Http-Finnova-Authorization')) {
            if (!$authToken) {
    ...
    protected function getUser2FAMethod(\Finnova\Entities\User $user) : ?string
    {
