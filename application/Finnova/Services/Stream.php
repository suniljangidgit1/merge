...
class Stream extends \Finnova\Core\Services\Base
{
    ...
    protected function getOnlyTeamEntityTypeList(\Finnova\Entities\User $user)
    {
    ...
    protected function getOnlyOwnEntityTypeList(\Finnova\Entities\User $user)
    {
        if ($user->isPortal()) return [];
    ...
    protected function getNotAllEntityTypeList(\Finnova\Entities\User $user)
    {
        if (!$user->isPortal()) return [];
    ...
    protected function getIgnoreScopeList(\Finnova\Entities\User $user)
    {
        $ignoreScopeList = [];
        $scopes = $this->getMetadata()->get('scopes', []);
        ...
        if ($user->isPortal() && !$this->getUser()->isPortal()) {
            $aclManager = new \Finnova\Core\Portal\AclManager($this->getInjection('container'));
