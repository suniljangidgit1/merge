...
namespace Finnova\Core\Utils\Authentication;
...
class LDAP extends Finnova
{
     ...
     * @param  \Finnova\Entities\AuthToken $authToken
     *
     * @return \Finnova\Entities\User | null
     */
    public function login($username, $password, \Finnova\Entities\AuthToken $authToken = null, $params = [], $request)
    {
        if (!$password) return;
        ...
        try {
            ...
            $GLOBALS['log']->info('LDAP: Administrator ['.$username.'] was logged in by Finnova method.');
        }
        ...
        if (!isset($user) && $this->getUtils()->getOption('createFinnovaUser')) {
     ...
     * @return \Finnova\Entities\User | null
     */
    protected function loginByToken($username, \Finnova\Entities\AuthToken $authToken = null)
    {
        if (!isset($authToken)) {
            return null;
        }
    ...
    /**
     * Create Finnova user with data gets from LDAP server
     *
     * @param  array $userData LDAP entity data
     * @param  boolean $isPortal Is portal user
     *
     * @return \Finnova\Entities\User
     */
    protected function createUser(array $userData, $isPortal = false)
    {
        ...
        foreach ($ldapFields as $finnova => $ldap) {
            $ldap = strtolower($ldap);
            if (isset($userData[$ldap][0])) {
                $GLOBALS['log']->debug('LDAP: Create a user wtih ['.$finnova.'] = ['.$userData[$ldap][0].'].');
                $data[$finnova] = $userData[$ldap][0];
            }
        }
     ...
     *
     * @param  string $filter E.g. "memberof=CN=externalTesters,OU=groups,DC=finnova,DC=local"
     *
     * @return string
     */
