...
class HookManager
{
    ...
    protected $paths = [
        'corePath' => 'application/Finnova/Hooks',
        'modulePath' => 'application/Finnova/Modules/{*}/Hooks',
        'customPath' => 'custom/Finnova/Custom/{*}/Hooks',
    ];
	...
	$customDataPath = str_replace('{*}', $cacheDataPath, $this->cacheFile);
        ...
        if ($this->getConfig()->get('useCache') && file_exists($customDataPath)) {
	...
	$customPath = str_replace('{*}', $db, $this->paths['customPath']);
        ...
        $data = $this->getHookData($customPath);
        ...
        if ($this->getConfig()->get('useCache')) {
            $this->getFileManager()->putPhpContents($customDataPath, $this->data);
        }
    }
     ...
     *
     * @param array $hookDirs - it can be an array('Finnova/Hooks', 'Finnova/Custom/Hooks', 'Finnova/Modules/Crm/Hooks')
     *
     * @return array
     */
