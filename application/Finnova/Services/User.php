...
class User extends Record
{
    ...
    public function changePassword($userId, $password, $checkCurrentPassword = false, $currentPassword = null)
    {
        ...
        if (!$user->isAdmin() && $this->getConfig()->get('authenticationMethod', 'Finnova') !== 'Finnova') {
            throw new Forbidden();
        }
        ...
        if ($checkCurrentPassword) {
            $passwordHash = new \Finnova\Core\Utils\PasswordHash($this->getConfig());
    ...
    protected function hashPassword($password)
    {
        ...
        $passwordHash = new \Finnova\Core\Utils\PasswordHash($config);
        ...
        if (!is_null($newPassword) && !empty($data->sendAccessInfo)) {
            if ($user->isActive()) {
                try {
                    ...
                    $this->sendPassword($user, $newPassword, 'create');
                } catch (\Exception $e) {}
            }
        }
        ...
        if (!is_null($newPassword)) {
            try {
                if ($user->isActive() && !empty($data->sendAccessInfo)) {
                    // $this->sendPassword($user, $newPassword);
                    $this->sendPassword($user, $newPassword, 'update');
                }
            } catch (\Exception $e) {}
        }
    ...
    {
        $apiKeyUtil = new \Finnova\Core\Utils\ApiKey($this->getConfig());
    ...
    public function generateNewApiKeyForEntity($id)
    {
        ...
        $apiKey = \Finnova\Core\Utils\Util::generateApiKey();
    ...
    protected function sendPassword(Entity $user, $password, $requestType = "")
    {
        ...
        $emailData["siteUrl"]       = $this->getConfig()->getSiteUrl() . '/';
        $emailData["emailAddress"]  = $emailAddress;
        $emailData["userName"]      = $userName;
        $emailData["firstName"]     = ucwords($firstName);
        $emailData["lastName"]      = ucwords($lastName);
        $emailData["password"]      = $password;

        if( !empty($requestType) ) {

            // CUSTOM EMAIL SEND FUNCTIONALITY

            $emailData["emailSubject"]   = "Congratulations | Welcome to FinCRM";
            $emailData["emailBody"]      = "Your user access has been created to FinCRM. These are your login details:";

            if( $requestType == "update" ) {

                $emailData["emailSubject"]   = "FinCRM Password Reset Successfully";
                $emailData["emailBody"]      = "Your FinCRM password has been reset successfully. These are your new details:";
            }

            require_once($_SERVER['DOCUMENT_ROOT']."/task_cron/email_templates/userCreateUpdateTemplate.php");

        }else{

            // EXSTING CRM FUNCTIONALITY
            ...
            $htmlizer = new \Finnova\Core\Htmlizer\Htmlizer($this->getFileManager(), $this->getDateTime(), $this->getNumber(), null);

            $subject = $htmlizer->render($user, $subjectTpl, null, $data, true);
            $body = $htmlizer->render($user, $bodyTpl, null, $data, true);die;
	...
	include('application/Finnova/Core/Utils/myconfig.php');
        $this->getFileManager()->removeFile('data/cache/'. $cacheDataPath .'/application/acl/' . $id . '.php');
    ...
    {
	include('application/Finnova/Core/Utils/myconfig.php');
        $this->getInjection('fileManager')->removeInDir('data/cache/'.$cacheDataPath.'/application/acl-portal');
    }
