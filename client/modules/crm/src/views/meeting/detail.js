...
Finnova.define('crm:views/meeting/detail', 'views/detail', function (Dep) {
    ...
    return Dep.extend({
        ...
        actionSendInvitations: function () {
            this.confirm(this.translate('confirmation', 'messages'), function () {
                ...
                this.notify('Sending...');
                Finnova.Ajax.postRequest(this.model.entityType + '/action/sendInvitations', {
                    id: this.model.id
                }).then(function (result) {
                    if (result) {
                        this.notify('Sent', 'success');
                    } else {
                        Finnova.Ui.warning(this.translate('nothingHasBeenSent', 'messages', 'Meeting'));
                    }
        ...
        actionSetAcceptanceStatus: function () {
            ...
            this.createView('dialog', 'crm:views/meeting/modals/acceptance-status', {
                ...
                this.listenTo(view, 'set-status', function (status) {
                    this.removeMenuItem('setAcceptanceStatus');
                    Finnova.Ajax.postRequest(this.model.entityType + '/action/setAcceptanceStatus', {
                        id: this.model.id,
                        status: status
                    }).then(function () {
                        this.model.fetch();
