...
class Smsreport extends CI_Controller {
	...
	public function __construct(){
		...
		$this->load->model("SmsGraphModel");
        $url    = $_SERVER["HTTP_HOST"];
        $domain = explode(".", $url);
        $domain = $domain[0];
        $domainStatus = $this->SmsGraphModel->getDomainStatus($domain);
	...
	public function index(){
		...
		if( !empty($userRole) ){ 
			...
			if( !empty($reportType) && $reportType == "y" ){
				...
				$data['xTitle']	= "Years";
				$result 		= $this->SmsGraphModel->yearData($startDate, $endDate, $multiUsers, $whereArr);
			}
			...
			// GET MONTH LIST
				...
				$data['xTitle']	= "Months";
				$result 		= $this->SmsGraphModel->monthData($startDate, $endDate, $multiUsers, $whereArr);
			}
			...
			// GET WEEK LIST
				...
				$data['xTitle']	= "Weeks";
				$result 		= $this->SmsGraphModel->weekData($startDate, $endDate, $multiUsers, $whereArr);
			}
			...
			// GET DAY LIST
				...
				$data['xTitle']	= "Days";
				$result 		= $this->SmsGraphModel->dayData($startDate, $endDate, $multiUsers, $whereArr);
			}
			...
			// GET HOUR LIST
				...
				$data['xTitle']	= "Hours";
				$result 		= $this->SmsGraphModel->hourData($startDate, $endDate, $multiUsers, $whereArr);
			}
			...
			// GET LAST 30 DAYS LIST
				...
				$result 		= $this->SmsGraphModel->dayData($tempStart, $tempEnd, $multiUsers, $whereArr);
			}
