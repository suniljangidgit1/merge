...
class Metadata
{
    ...
    protected $pathToModules = 'application/Finnova/Modules';
    ...
    protected $paths = array(
        'corePath' => 'application/Finnova/Resources/metadata',
        'modulePath' => 'application/Finnova/Modules/{*}/Resources/metadata',
        'customPath' => 'custom/Finnova/Custom/{*}/Resources/metadata',
    );
    ...
    public function __construct(\Finnova\Core\Utils\File\Manager $fileManager, $useCache = false)
    {
    ...
    protected function getUnifier()
    {
        if (!isset($this->unifier)) {
            $this->unifier = new \Finnova\Core\Utils\File\Unifier($this->fileManager, $this, false);
        }
    ...
    protected function getObjUnifier()
    {
        if (!isset($this->objUnifier)) {
            $this->objUnifier = new \Finnova\Core\Utils\File\Unifier($this->fileManager, $this, true);
        }
    ...
    protected function getModuleConfig()
    {
        if (!isset($this->moduleConfig)) {
            $this->moduleConfig = new \Finnova\Core\Utils\Module($this->fileManager, $this->useCache);
        }
	...
	$customDataPath = str_replace('{*}', $cacheDataPath, $this->cacheFile);
        ...
        if (file_exists($customDataPath) && !$reload) {
            ...
            if ($this->useCache) {
                $isSaved = $this->getFileManager()->putPhpContents($customDataPath, $this->data);
                if ($isSaved === false) {
    ...
    */
	...
	include('application/Finnova/Core/Utils/myconfig.php');
	$customDataPath = str_replace('{*}', $cacheDataPath, $this->objCacheFile);

        if (file_exists($customDataPath) && !$reload) {
            $this->objData = $this->getFileManager()->getPhpContents($customDataPath);
        } else {
            $this->objData = $this->getObjUnifier()->unify('metadata', $this->paths, true);
            $this->objData = $this->addAdditionalFieldsObj($this->objData);

            if ($this->useCache) {
                $isSaved = $this->getFileManager()->putPhpContents($customDataPath, $this->objData, true);
                if ($isSaved === false) {
    ...
    */
	...
	$customPath = str_replace('{*}', $db, $this->paths['customPath']);
        $filePath = array($customPath, $key1, $key2.'.json');
    ...
    */
    ...
    {
	include('application/Finnova/Core/Utils/myconfig.php');
	$path = str_replace('{*}', $db, $this->paths['customPath']);
    ...
    /**
     * Get Entity path, ex. Finnova.Entities.Account or Modules\Crm\Entities\MyModule
     *
    ...
    public function getScopePath($scopeName, $delim = '/')
    {
        ...
        $path = ($moduleName !== false) ? 'Finnova/Modules/'.$moduleName : 'Finnova';
