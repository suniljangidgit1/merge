<?php
	...
	/////////// CODE FOR GET DATA FOR EMAIL ID's //////////////////////////////////////////////
	$sql_for_get_email_add_id= "SELECT * FROM entity_email_address WHERE entity_id='".$id."'";
	$result_for_get_email_add_id= mysqli_query($conn, $sql_for_get_email_add_id);
	
	while($row_for_get_email_add_id= mysqli_fetch_array($result_for_get_email_add_id)){
		$email_address_id= $row_for_get_email_add_id['email_address_id'];
		$entity_type= $row_for_get_email_add_id['entity_type'];
		$primary= $row_for_get_email_add_id['primary'];
	    
	    echo "EMAIL ID FROM ENTITY EMAIL ADDRESS= ".$email_address_id;
	    echo "<br>";
	    
	    echo "ENTITY TYPE= ".$entity_type;
	    echo "<br>";
	    	
		$sql_for_get_email_add= "SELECT * FROM email_address WHERE id='".$email_address_id."'";
		$result_for_get_email_add= mysqli_query($conn, $sql_for_get_email_add);
		$row_for_get_email_add= mysqli_fetch_array($result_for_get_email_add);
		
		$email_add_name= $row_for_get_email_add['name'];
		$email_add_lower= $row_for_get_email_add['lower'];
		
		echo "<br>";
		echo "EMAIL ADDRESS= ".$email_add_name;
		echo "<br>";
		echo "EMIAL ADDRESS IN LOWER= ".$email_add_lower;
		echo "<br>";
		
		$new_id_for_email_add_table= getToken($length);
		mysqli_query($conn, "INSERT INTO email_address(`id`, `name`, `deleted`, `lower`, `invalid`, `opt_out`) VALUES ('".$new_id_for_email_add_table."', '".$email_add_name."', 0, '".$email_add_lower."', 0, 0)");
		
		mysqli_query($conn, "INSERT INTO entity_email_address (`entity_id`, `email_address_id`, `entity_type`, `primary`, `deleted`) VALUES ('".$new_id_for_entity."', '".$new_id_for_email_add_table."', '".$entity_type."', '".$primary."', 0)");
	...
	/////////// CODE FOR GET DATA FOR PHONE NUMBER //////////////////////////////////////////////
	...
	while($row_for_get_phone_number = mysqli_fetch_array($result_for_get_phone_number)){
		...
		echo "<br>";
		echo "ENTITY PHONE NUMBER ID IN ENTITY PHONE NUMBER TABLE==".$phone_number_id;
		echo "<br>";
		...
		echo "PHONE NUMBER FROM PHONE NUMBER TABLE= ".$entity_phone_number;
		echo "<br>";
		echo "PHONE NUMBER TYPE FROM PHONE NUMBER TABLE= ".$phone_number_type;
		echo "<br>";
		echo $new_id_for_phone_number;
		echo "<br>";
		mysqli_query($conn, "INSERT INTO phone_number (id, name, deleted, type) VALUES ('".$new_id_for_phone_number."', '".$entity_phone_number."', '0', '".$phone_number_type."')");
		
		//$new_id_for_entity_for_entity_phone_number= getToken($length);
		mysqli_query($conn,"INSERT INTO entity_phone_number (`entity_id`, `phone_number_id`, `entity_type`, `primary`, `deleted`) VALUES ('".$new_id_for_entity."', '".$new_id_for_phone_number."', '".$entity_type."', 0, 0) ");
	...
	echo "<br>";
	echo "ENTITY ID OUTSIDE THE EMAIL AND PHONE WHILE LOOP== ". $new_id_for_entity;
	echo "<br>";
