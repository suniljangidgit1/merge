...
use \Finnova\Core\Exceptions\Conflict;
...
class EntityManager
{
    ...
    public function __construct(Metadata $metadata, Language $language, File\Manager $fileManager, Config $config, Container $container = null)
    {
        ...
        $this->metadataHelper = new \Finnova\Core\Utils\Metadata\Helper($this->metadata);
	...
	include('application/Finnova/Core/Utils/myconfig.php');
        $controllerClassName = '\\Finnova\\Custom\\Controllers\\' . Util::normilizeClassName($name);
        if (class_exists($controllerClassName)) {
            return true;
        } else {
            foreach ($this->getMetadata()->getModuleList() as $moduleName) {
                $controllerClassName = '\\Finnova\\Modules\\' . $moduleName . '\\Controllers\\' . Util::normilizeClassName($name);
                if (class_exists($controllerClassName)) {
                    return true;
                }
            }
            $controllerClassName = '\\Finnova\\Controllers\\' . Util::normilizeClassName($name);
            if (class_exists($controllerClassName)) {
                return true;
            }
        }
	...
	include('application/Finnova/Core/Utils/myconfig.php');
        ...
        $templateNamespace = "\Finnova\Core\Templates";
        $templatePath = "application/Finnova/Core/Templates";
        ...
        if (!empty($templateDefs['module'])) {
            ...
            $templateNamespace = "\Finnova\Modules\\{$normalizedTemplateModuleName}\Core\Templates";
            $templatePath = "application/Finnova/Modules/".$normalizedTemplateModuleName."/Core/Templates";
        }

        $contents = "<" . "?" . "php\n\n".
            "namespace Finnova\Custom\\".$db."\Entities;\n\n".
            "class {$normalizedName} extends {$templateNamespace}\Entities\\{$type}\n".
            "{\n".
            "    protected \$entityType = \"$name\";\n".
            "}\n";

        $filePath = "custom/Finnova/Custom/{$db}/Entities/{$normalizedName}.php";
        $this->getFileManager()->putContents($filePath, $contents);

        $contents = "<" . "?" . "php\n\n".
            "namespace Finnova\Custom\\".$db."\Controllers;\n\n".
            "class {$normalizedName} extends {$templateNamespace}\Controllers\\{$type}\n".
            "{\n".
            "}\n";
        $filePath = "custom/Finnova/Custom/".$db."/Controllers/{$normalizedName}.php";
        $this->getFileManager()->putContents($filePath, $contents);

        $contents = "<" . "?" . "php\n\n".
            "namespace Finnova\Custom\\".$db."\Services;\n\n".
            "class {$normalizedName} extends {$templateNamespace}\Services\\{$type}\n".
            "{\n".
            "}\n";
        $filePath = "custom/Finnova/Custom/{$db}/Services/{$normalizedName}.php";
        $this->getFileManager()->putContents($filePath, $contents);

        $contents = "<" . "?" . "php\n\n".
            "namespace Finnova\Custom\\".$db."\Repositories;\n\n".
            "class {$normalizedName} extends {$templateNamespace}\Repositories\\{$type}\n".
            "{\n".
            "}\n";

        $filePath = "custom/Finnova/Custom/{$db}/Repositories/{$normalizedName}.php";
        $this->getFileManager()->putContents($filePath, $contents);

        if (file_exists($templatePath . '/SelectManagers/' . $type . '.php')) {
            $contents = "<" . "?" . "php\n\n".
                "namespace Finnova\Custom\\".$db."\SelectManagers;\n\n".
                "class {$normalizedName} extends {$templateNamespace}\SelectManagers\\{$type}\n".
                "{\n".
                "}\n";

            $filePath = "custom/Finnova/Custom/{$db}/SelectManagers/{$normalizedName}.php";
            $this->getFileManager()->putContents($filePath, $contents);
        }
        ...
        foreach ($languageList as $language) {
            ...
            $destinationFilePath = 'custom/Finnova/Custom/'.$db.'/Resources/i18n/' . $language . '/' . $name . '.json';
        ...
        if ($this->getFileManager()->isDir($layoutsPath)) {
            $this->getFileManager()->copy($layoutsPath, 'custom/Finnova/Custom/'.$db.'/Resources/layouts/' . $name);
        }

        $this->processHook('afterCreate', $type, $name, $params);

        // My Code FOr Create Entity Master Table .....
        $user = "root"; 
        $password = "root"; 
        $host = "localhost"; 
        // $dbase = $db; 
        ...
        $exists = mysqli_query($connection, "select * from entity_master");

         $entityName= $name;
         $entityTableName= strtolower(preg_replace('/\B([A-Z])/', '_$1', $name));
         $entityOriginalName= $name;
         // $createAt=  
         // $createdBy= $_SESSION['Login'];   
         $createdBy= "admin";   
          // echo "ENTITY NAME = ". $entityName. "TABLE NAME = ". $entityTableName; //die;   

        if($exists !== FALSE)
        {
            // echo "ON IF BLOCK";
             // echo "insert into entity_master (`entity_name`, `table_name`, `entity_name_original`, `created_by_id`) values ('$entityName', '$entityTableName', '$entityOriginalName', '$createdBy')";
           mysqli_query($connection, "insert into entity_master (`entity_name`, `table_name`, `entity_name_original`, `created_by_id`) values ('$entityName', '$entityTableName', '$entityOriginalName', '$createdBy')"); 
            ...
            // die;
               ...
               // $sql1=  ;
            $sql = mysqli_query ($connection,"create table entity_master (id int(11) NOT NULL AUTO_INCREMENT,
                entity_name varchar(50) DEFAULT NULL,
                table_name varchar(50) DEFAULT NULL,
                entity_name_original varchar(50) DEFAULT NULL,
                created_at varchar(50) DEFAULT NULL,
                created_by_id varchar(50) DEFAULT CURRENT_TIMESTAMP,
                PRIMARY KEY (id))ENGINE=InnoDB AUTO_INCREMENT=86 DEFAULT CHARSET=utf8");
	...
	include('application/Finnova/Core/Utils/myconfig.php');
        $this->getFileManager()->removeFile("custom/Finnova/Custom/{$db}/Resources/metadata/entityDefs/{$name}.json");
        $this->getFileManager()->removeFile("custom/Finnova/Custom/{$db}/Resources/metadata/clientDefs/{$name}.json");
        $this->getFileManager()->removeFile("custom/Finnova/Custom/{$db}/Resources/metadata/scopes/{$name}.json");

        $this->getFileManager()->removeFile("custom/Finnova/Custom/{$db}/Entities/{$normalizedName}.php");
        $this->getFileManager()->removeFile("custom/Finnova/Custom/{$db}/Services/{$normalizedName}.php");
        $this->getFileManager()->removeFile("custom/Finnova/Custom/{$db}/Controllers/{$normalizedName}.php");
        $this->getFileManager()->removeFile("custom/Finnova/Custom/{$db}/Repositories/{$normalizedName}.php");

        if (file_exists("custom/Finnova/Custom/{$db}/SelectManagers/{$normalizedName}.php")) {
            $this->getFileManager()->removeFile("custom/Finnova/Custom/{$db}/SelectManagers/{$normalizedName}.php");
        }

        $this->getFileManager()->removeInDir("custom/Finnova/Custom/{$db}/Resources/layouts/{$normalizedName}");
        $this->getFileManager()->removeDir("custom/Finnova/Custom/{$db}/Resources/layouts/{$normalizedName}");

        $languageList = $this->getConfig()->get('languageList', []);
        foreach ($languageList as $language) {
            $filePath = 'custom/Finnova/Custom/'.$db.'/Resources/i18n/' . $language . '/' . $normalizedName . '.json' ;
            if (!file_exists($filePath)) continue;
    ...
    protected function getHook($type)
    {
        ...
        $className = '\\Finnova\\Core\\Utils\\EntityManager\\Hooks\\' . $type . 'Type';

        $templateModuleName = null;
        if (!empty($templateDefs['module'])) {
            $templateModuleName = $templateDefs['module'];
            $normalizedTemplateModuleName = Util::normilizeClassName($templateModuleName);
            $className = '\\Finnova\\Modules\\'.$normalizedTemplateModuleName.'\\Core\\Utils\\EntityManager\\Hooks\\' . $type . 'Type';
        }
