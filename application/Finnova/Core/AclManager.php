...
class AclManager
{
    ...
    protected $tableClassName = '\\Finnova\\Core\\Acl\\Table';

    protected $userAclClassName = '\\Finnova\\Core\\Acl';
    ...
    public function __construct(Container $container)
    {
        ...
        $this->globalRestricton = new \Finnova\Core\Acl\GlobalRestricton(
    ...
    public function getImplementation($scope)
    {
        if (empty($this->implementationHashMap[$scope])) {
            ...
            $className = '\\Finnova\\Custom\\Acl\\' . $normalizedName;
            if (!class_exists($className)) {
                $moduleName = $this->metadata->getScopeModuleName($scope);
                if ($moduleName) {
                    $className = '\\Finnova\\Modules\\' . $moduleName . '\\Acl\\' . $normalizedName;
                } else {
                    $className = '\\Finnova\\Acl\\' . $normalizedName;
                }
                if (!class_exists($className)) {
                    $className = '\\Finnova\\Core\\Acl\\Base';
                }
            }
