...
use \Finnova\Core\Exceptions\ForbiddenSilent;
use \Finnova\Core\Exceptions\ConflictSilent;

use \Finnova\Core\Utils\Util;

class Record extends \Finnova\Core\Services\Base
{
    ...
    public function setAcl(\Finnova\Core\Acl $acl)
    {
        $this->acl = $acl;
    }
    ...
    public function getListKanban($params)
    {
        ...
        $collection = new \Finnova\ORM\EntityCollection([], $this->entityType);
    ...
    public function findDuplicates(Entity $entity, $data = null) : ?\Finnova\ORM\EntityCollection
    {
        if (!$data) {
            $data = (object) [];
        }
        ...
        if (!empty($defs[$attribute]) && !empty($defs[$attribute]['type'])) {
            ...
            switch ($type) {
                case 'jsonObject':
                    ...
                    return \Finnova\Core\Utils\Json::encode($value, \JSON_UNESCAPED_UNICODE);
                    break;
                case 'jsonArray':
    ...
    public function getDuplicateAttributes($id)
    {
        ...
        $fieldManager = new \Finnova\Core\Utils\FieldManagerUtil($this->getMetadata());
