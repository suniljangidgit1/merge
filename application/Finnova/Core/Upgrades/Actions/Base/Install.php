...
class Install extends \Finnova\Core\Upgrades\Actions\Base
{
    /**
    ...
    public function stepCopy(array $data)
    {
        ...
        /* copy files from directory "Files" to FinnovaCRM files */
        if (!$this->copyFiles()) {
    ...
    public function stepRebuild(array $data)
    {
        ...
        if (!isset($data['skipSystemRebuild']) || !$data['skipSystemRebuild']) {
            if (!$this->systemRebuild()) {
                $this->throwErrorAndRemovePackage('Error occurred while FinnovaCRM rebuild. Please see the log for more detail.');
            }
        }
