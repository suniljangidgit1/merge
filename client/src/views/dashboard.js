    ...
    return Dep.extend({
        ...
        setupCurrentTabLayout: function () {
            if (!this.dashboardLayout) {
                ...
                if (this.getConfig().get('forcedDashboardLayout')) {
                    ...
                    var tempStore       = [];
                    var ajaxResponse    = this.checkEntityAccess();
                    var configLayout    = this.getPreferences().get('dashboardLayout'); 
                    
                    if( ajaxResponse.userRole != "admin" ) {
                        
                        $( configLayout ).each(function(key, value ) {
                            
                            if( ajaxResponse.userRole != "admin" && value["name"] == "Users" ) {
                                
                            }else if( ajaxResponse.Lead == "false" && value["name"] == "Leads" ) {
                                
                            }else if( ajaxResponse.Opportunity == "false" && value["name"] == "Opportunities" ) {
                        ...
                        tempStore = this.getPreferences().get('dashboardLayout');
                    }

                    // this.dashboardLayout = this.getPreferences().get('dashboardLayout') || defaultLayout; // EXISTING CODE HERE
                    this.dashboardLayout = tempStore || defaultLayout; 
        ...
        prepareGridstackItem: function (id, name) {
            ...
            if(name=="addWindow"){
                ...
                //       $(".dashlet-body").html(result);
                //     }
                // });


                var $item = $('<div></div>');
                var $container = $('<div class="grid-stack-item-content dashlet-container LeadListData"  style="height:200px; background: #fff;"></div>');
        ...
        checkEntityAccess: function (){

            var entityAccess = "";
            $.ajax({
                url         : "../../../../../client/res/templates/custom_dashboard/dashboardAjaxController.php",
                type        : "GET",
                dataType    : "JSON",
                data        : { methodName : "checkEntityAccess", parameters : "false" },
                "async"     : false,
                // processData : false,
                // contentType : false,
