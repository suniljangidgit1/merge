...
class Unifier
{
    ...
    protected $pathToDefaults = 'application/Finnova/Core/defaults';

    public function __construct(\Finnova\Core\Utils\File\Manager $fileManager, \Finnova\Core\Utils\Metadata $metadata = null, $useObjects = false)
    {
	...
	$customPath = str_replace('{*}', $db, $paths['customPath']);
        if (!empty($customPath)) {
            if ($this->useObjects) {
                $content = Utils\DataUtil::merge($content, $this->unifySingle($customPath, $name, $recursively));
            } else {
                $content = Utils\Util::merge($content, $this->unifySingle($customPath, $name, $recursively));
            }
        }
