...
$load_lang_code = "en";
require($_SERVER['DOCUMENT_ROOT'].'/client/ckeditor/lang/en.php');
// Including the plugin config file, don't delete the following row!
require($_SERVER['DOCUMENT_ROOT'].'/client/ckeditor/pluginconfig.php');
// Including the functions file, don't delete the following row!
require($_SERVER['DOCUMENT_ROOT'].'/client/ckeditor/function.php');
// Including the check_permission file, don't delete the following row!
require($_SERVER['DOCUMENT_ROOT'].'/client/ckeditor/check_permission.php');

if ($username == "" and $password == "") {
    if(!isset($_SESSION['username'])){
        include($_SERVER['DOCUMENT_ROOT'].'/client/ckeditor/new.php');
        exit;	
    }
} else {
    if(!isset($_SESSION['username'])){
        include($_SERVER['DOCUMENT_ROOT'].'/client/ckeditor/loginindex.php');
        exit;	
    }
}
...
Select Type : <input type="radio" class="rtype" name="loadtype" value="img" checked <?php if(!empty($_POST['loadtype'])) if($_POST['loadtype'] == "img") echo "checked";?> >&nbsp;Image &nbsp;&nbsp;
  <input type="radio" class="rtype" name="loadtype" value="audio" <?php if(!empty($_POST['loadtype'])) if($_POST['loadtype'] == "audio") echo "checked";?>>&nbsp; Audio &nbsp;&nbsp;
  <input type="radio" class="rtype" name="loadtype" value="video" <?php if(!empty($_POST['loadtype'])) if($_POST['loadtype'] == "video") echo "checked";?>>&nbsp;Video &nbsp;&nbsp;
 <br>
 </form>
<!----><!---->
...
$('.rtype').on('change', function() {
        	var val =  $('input[name=loadtype]:checked').val();
		   	$('input[name=loadtype][value="' + val + '"]').attr('checked', true);
		    $("#subform").submit();
...
<div id="files">
    <?php
     if(isset($_POST['loadtype'])){
		$selected_val = $_POST['loadtype'];  // Storing Selected Value In Variable
		loadImages($selected_val);
	}
	else{
		loadImages('img');
	}
