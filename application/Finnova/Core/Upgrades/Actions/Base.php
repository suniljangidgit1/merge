...
namespace Finnova\Core\Upgrades\Actions;

use Finnova\Core\Utils\Util;
use Finnova\Core\Utils\System;
...
abstract class Base
{
    ...
    public function __construct(\Finnova\Core\Container $container, \Finnova\Core\Upgrades\ActionManager $actionManager)
    {
        $this->container = $container;
        $this->actionManager = $actionManager;
        $this->params = $actionManager->getParams();

        $this->zipUtil = new \Finnova\Core\Utils\File\ZipArchive($container->get('fileManager'));
    }
    ...
    /**
     * Check if version of upgrade/extension is acceptable to current version of FinnovaCRM
     *
    ...
    protected function isAcceptable()
    {
        ...
        if (isset($manifest['acceptableVersions'])) {
            $res &= $this->checkVersions($manifest['acceptableVersions'], $this->getConfig()->get('version'), 'Your FinnovaCRM version ({version}) is not supported. Required version: {requiredVersion}.');
        }
