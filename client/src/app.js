...
define(
    ...
    var App = function (options, callback) {
        ...
        this.id = options.id || 'finnovacrm-application-id';
    ...
    _.extend(App.prototype, {
            ...
            if (!(name in this.controllers)) {
                try {
                    ...
                    if (!className) {
                        ...
                        className = Finnova.Utils.composeClassName(module, name, 'controllers');
                    }
                    Finnova.require(className, function (controllerClass) {
        ...
        initView: function () {
            ...
            this.viewLoader = function (viewName, callback) {
                Finnova.require(Finnova.Utils.composeViewClassName(viewName), callback);
            }.bind(this);
        ...
        logout: function () {
            if (this.auth) {
                ...
                if (arr.length > 1) {
                    Finnova.Ajax.postRequest('App/action/destroyAuthToken', {
                        token: arr[1]
                    });
                }
            }
        ...
        requestUserData: function (callback) {
            Finnova.Ajax.getRequest('App/user').then(callback);
        },
            ...
            $.ajaxSetup({
                beforeSend: function (xhr, options) {
                    if (!options.local && self.url) {
                        options.url = Finnova.Utils.trimSlash(self.url) + '/' + options.url;
                    }
                    ...
                    if (self.auth !== null) {
                        ...
                        xhr.setRequestHeader('Finnova-Authorization', self.auth);
                        xhr.setRequestHeader('Finnova-Authorization-By-Token', true);
                    }
                },
                dataType: 'json',
                timeout: 60000,
                ...
                switch (xhr.status) {
                    case 0:
                        if (xhr.statusText == 'timeout') {
                            Finnova.Ui.error(self.language.translate('Timeout'));
                        }
                        break;
                    case 200:
                        Finnova.Ui.error(self.language.translate('Bad server response'));
                    ...
                    case 401:
                        if (!options.login) {
                            if (self.auth) {
                                ...
                                Finnova.Ui.error(self.language.translate('Auth error'));
                            }
                        }
                        break;
                    case 403:
