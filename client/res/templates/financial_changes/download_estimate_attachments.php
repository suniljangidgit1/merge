<?php
    ...
    $result=$s3->getObject(
    	array(
    		  'Bucket'=>'fincrm',
    		  'Key'=>'Development/'.$_SERVER['SERVER_NAME'].'/financial_files/estimates/'.$row1['user_name'].'/'.$estid.'/'.$row1["filename"],
    		  'SaveAs'=>$path.'/'.$row1["filename"]
    		)
    );

    $zip=new ZipArchive;
    // $file_path='uploads/financial_files/'.$name.'.zip';
    $file_path='financial_files/estimate/zipFolder/'.$row1["filename"];
    $res=$zip->open($file_path);
    if($res===TRUE)
    {
    	// $zip->extractTo('uploads/financial_files/');
        $zip->extractTo($_SERVER['DOCUMENT_ROOT'].'/client/res/templates/financial_files/estimate/uploads/');
    	$zip->close();
    }
    else{
    	echo "error to unzip";
