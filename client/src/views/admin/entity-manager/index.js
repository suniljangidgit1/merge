...
Finnova.define('views/admin/entity-manager/index', 'view', function (Dep) {
    ...
    return Dep.extend({
        ...
        setupScopeData: function () {
            ...
            scopeList.forEach(function (scope) {
                var d = this.getMetadata().get('scopes.' + scope);
                var entityNameArr = ["BillingEntity", "Campaign", "Closed Task", "Contact Lists", "Document", "Document Folder", "Email", "Email Reminder", "Email Template Categories", "Estimate", "Export", "Export Result", "Invoice", "Knowledge Base Article", "Knowledge Base Category", "Message Logs", "My Campaigns",  "Payments", "Sent SMS Data", "Sent Email Reminder", "Sent Messages", "SMS Reminder", "Template", "Array Value", "Attachment", "Auth Log Record", "Auth Token", "Campaign Log Record", "Tracking URL", "Currency", "Personal Email Account", "Email Address", "Email Filter", "Email Queue Item", "Email Template", "Extension", "External Account", "Import", "Group Email Account", "Integration", "Job", "Lead Capture Entry Point", "Lead Capture Log Record", "Mass Email", "Note", "Notification", "Password Change Request", "Phone Number", "Portal", "Portal Role", "Preferences", "Reminder", "Role", "Scheduled Job", "Scheduled Job Log Record", "Target List", "Team", "Unique ID", "UserData", "Webhook", "WebhookEventQueueItem", "WebhookQueueItem", "HolidayCalender"];
                var entityName = this.getLanguage().translate(scope, 'scopeNames');
                if(entityNameArr.indexOf(this.getLanguage().translate(scope, 'scopeNames')) !== -1 ){
                    ...
                    if(this.getLanguage().translate(scope, 'scopeNames')==="Office Location"){
                            isRemovable = false;
                    }
                    var edit = true;
