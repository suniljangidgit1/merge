...
class Layout
{
    ...
    protected $defaultsPath = 'application/Finnova/Core/defaults';

    protected $paths = [
        'corePath' => 'application/Finnova/Resources/layouts',
        'modulePath' => 'application/Finnova/Modules/{*}/Resources/layouts',
        'customPath' => 'custom/Finnova/Custom/{*}/Resources/layouts',
    ];

    public function __construct(\Finnova\Core\Utils\File\Manager $fileManager, \Finnova\Core\Utils\Metadata $metadata, \Finnova\Entities\User $user)
    {
    ...
    public function get(string $scope, string $name) : ?string
    {
        ...
        if (!file_exists($filePath)) {
            $defaultImplClassName = '\\Finnova\\Custom\\Core\\Utils\\Layout\\Defaults\\' . ucfirst($name) . 'Type';
            if (!class_exists($defaultImplClassName))
                $defaultImplClassName = '\\Finnova\\Core\\Utils\\Layout\\Defaults\\' . ucfirst($name) . 'Type';
	...
	include('application/Finnova/Core/Utils/myconfig.php');
        $filePath = 'custom/Finnova/Custom/'.$db.'/Resources/layouts/' . $scope . '/' . $name . '.json';
