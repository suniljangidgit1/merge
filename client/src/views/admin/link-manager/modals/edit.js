...
Finnova.define('views/admin/link-manager/modals/edit', ['views/modal', 'views/admin/link-manager/index', 'model'], function (Dep, Index, Model) {
    ...
    return Dep.extend({
        ...
        setup: function () {
            ...
            var entityListArr = ["Activities", "BillingEntity", "Campaign", "Designation", "DocumentFolder", "Document", "EmailTemplateCategory", "EmailTemplate", "Email", "Estimate", "ExternalAccount", "Invoice", "MessageLog", "NSICData", "Payments", "EmailAccountScope", "TargetList", "Template", "Webhook", "Export", "ExportResult", "HolidayCalender","Import", "OfficeLocation", "Team","ClosedTask","ContactList","EmailReminder","SentEmailReminder","SentMessages","SendSMSData","SMSReminder","MyCampaigns"];

            entityList = entityList.filter(function(val) {
              return entityListArr.indexOf(val) == -1;
            });
        ...
        populateFields: function () {
            ...
            switch (linkType) {
                case 'oneToMany':
                    linkForeign = Finnova.Utils.lowerCaseFirst(this.scope);
                    link = this.toPlural(Finnova.Utils.lowerCaseFirst(entityForeign));
                    if (entityForeign == this.scope) {

                        if (linkForeign == Finnova.Utils.lowerCaseFirst(this.scope)) {
                            linkForeign = linkForeign + 'Parent';
                        }
                    }
                    break;
                case 'manyToOne':
                    linkForeign = this.toPlural(Finnova.Utils.lowerCaseFirst(this.scope));
                    link = Finnova.Utils.lowerCaseFirst(entityForeign);
                    if (entityForeign == this.scope) {
                        if (link == Finnova.Utils.lowerCaseFirst(this.scope)) {
                            link = link + 'Parent';
                        }
                    }
                    break;
                case 'manyToMany':
                    ...
                    if (this.scope.localeCompare(entityForeign)) {
                        relationName = Finnova.Utils.lowerCaseFirst(this.scope) + entityForeign;
                    } else {
                        relationName = Finnova.Utils.lowerCaseFirst(entityForeign) + this.scope;
                    }
                    this.model.set('relationName', relationName);
                    break;
            }
            ...
            var label = Finnova.Utils.upperCaseFirst(link.replace(/([a-z])([A-Z])/g, '$1 $2'));
            var labelForeign = Finnova.Utils.upperCaseFirst(linkForeign.replace(/([a-z])([A-Z])/g, '$1 $2'));
        ...
        handleLinkChange: function (field) {
            ...
            if (value) {
                ...
                if (value.length) {
                     value = Finnova.Utils.lowerCaseFirst(value);
                }
            }
        ...
        save: function () {
            ...
            $.ajax({
                ...
                if (!this.isNew) {
                    Finnova.Ui.success(this.translate('Saved'));
                } else {
                    Finnova.Ui.success(this.translate('Created'));
                }
