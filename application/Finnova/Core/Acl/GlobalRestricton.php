...
class GlobalRestricton
{
    ...
    public function __construct(
        \Finnova\Core\Utils\Metadata $metadata,
        \Finnova\Core\Utils\File\Manager $fileManager,
        \Finnova\Core\Utils\FieldManagerUtil $fieldManagerUtil,
        bool $useCache = true
    )
    {
	...
	$customPath = str_replace('{*}', $cacheDataPath, $this->cacheFilePath);
        if ($useCache) {
            if (file_exists($customPath)) {
                $this->data = include($customPath);
                $isFromCache = true;
	...
	$customPath = str_replace('{*}', $cacheDataPath, $this->cacheFilePath);
        ...
        $this->getFileManager()->putPhpContents($customPath, $this->data, true);
    }
