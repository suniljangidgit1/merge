...
Finnova.define('views/note/fields/post', ['views/fields/text', 'lib!Textcomplete'], function (Dep, Textcomplete) {
            ...
            if (assignmentPermission !== 'no') {
                this.$element.textcomplete([{
                    ...
                    search: function (term, callback) {
                        ...
                        Finnova.Ajax.getRequest(buildUserListUrl(term)).then(function (data) {
                            callback(data.list)
                        });
                    },
            ...
            if (/^http(s){0,1}\:\/\//.test(text)) {
                ...
                if (regExp.test(text)) {
                    ...
                    this.ajaxPostRequest('Attachment/action/getAttachmentFromImageUrl', {
                        ...
                        var attachmentIdList = Finnova.Utils.clone(this.model.get('attachmentsIds') || []);
                        var attachmentNames = Finnova.Utils.clone(this.model.get('attachmentsNames') || {});
                        var attachmentTypes = Finnova.Utils.clone(this.model.get('attachmentsTypes') || {});
