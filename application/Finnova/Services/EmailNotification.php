...
class EmailNotification extends \Finnova\Core\Services\Base
{
    ...
    protected function getHtmlizer()
    {
        if (empty($this->htmlizer)) {
            $this->htmlizer = new \Finnova\Core\Htmlizer\Htmlizer($this->getInjection('fileManager'), $this->getInjection('dateTime'), $this->getInjection('number'), null);
        }
    ...
    protected function getSiteUrl(\Finnova\Entities\User $user)
    {
        if ($user->isPortal()) {
