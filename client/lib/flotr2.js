...
Text.prototype = {
  ...
  html : function (text, element, style, className) {
    ...
    D.setStyles(div, {'top' : '0px', 'left' : '-10000px' }); // FinnovaCRM fix line
...
Graph.prototype = {
  ...
  draw: function(after) {
    ...
    if (this.series.length) {
      ...
      for (i = 0; i < this.series.length; i++) {
        this.series[i].index = i; // FinnovaCRM fix stacked
  ...
  drawSeries: function(series){
    ...
    function drawChart (series, typeKey) {
      ...
      options.index = series.index; // FinnovaCRM fix stacked
...
Flotr.addType('bars', {
  ...
  plot : function (options) {
    ...
    for (i = 0; i < data.length; i++) {
      geometry = this.getBarGeometry(data[i][0], data[i][1], options, true); // FinnovaCRM fix stacked
  ...
  getBarGeometry : function (x, y, options, fillStack) { // FinnovaCRM fix stacked
    ...
    if (stack) {
      ...
      // FinnovaCRM fix stacked end

      if (fillStack) // FinnovaCRM fix stacked
    ...
    return (x === null || y === null) ? null : {
      bottom: bottom, // FinnovaCRM fix stacked
      x         : xValue,
      y         : yValue,
      xScale    : xScale,
      yScale    : yScale,
  ...
  drawHit : function (options) {
    ...
    if (!options.stacked) // FinnovaCRM fix stacked
...
Flotr.addPlugin('hit', {
  ...
  drawMouseTrack : function (n) {
    ...
    if (_.isNull(content) || _.isUndefined(content)) {
      ...
      // FinnovaCRM fix start
      if (this.options && this.options.mouse && this.options.mouse.cursorPointer) {
        $(this.el).css('cursor', 'pointer'); // FinnovaCRM fix
      }
      // FinnovaCRM fix end
    }
    ...
    // Pie
      ...
      var center = {
        ...
        radius = (Math.min(this.canvasWidth, this.canvasHeight) * s.pie.sizeRatio), // FinnovaCRM fix line
