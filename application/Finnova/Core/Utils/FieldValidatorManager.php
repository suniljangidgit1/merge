...
class FieldValidatorManager
{
    ...
    public function check(\Finnova\ORM\Entity $entity, string $field, string $type, $data = null) : bool
    {
        if (!$data) $data = (object) [];
    ...
    protected function loadImpl(string $fieldType)
    {
        ...
        if (!$className) {
            $className = '\\Finnova\\Core\\FieldValidators\\' . ucfirst($fieldType) . 'Type';
            if (!class_exists($className)) {
                $className = '\\Finnova\\Core\\FieldValidators\\BaseType';
            }
        }
