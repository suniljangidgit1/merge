...
define('views/record/search', 'view', function (Dep) {
    ...
    return Dep.extend({
        ...
        setup: function () {
            ...
            this.boolFilterList = Finnova.Utils.clone(this.getMetadata().get('clientDefs.' + this.scope + '.boolFilterList') || []).filter(function (item) {
                ...
                if (item.accessDataList) {
                    if (!Finnova.Utils.checkAccessDataList(item.accessDataList, this.getAcl(), this.getUser())) {
                        return false;
                    }
                }
                return true;
            ...
            this.presetFilterList = Finnova.Utils.clone(filterList).filter(function (item) {
                if (typeof item === 'string') return true;
                item = item || {};
                if (item.inPortalDisabled && this.getUser().isPortal()) return false;
                if (item.isPortalOnly && !this.getUser().isPortal()) return false;
        ...
        getPresetData: function () {
            ...
            this.getPresetFilterList().forEach(function (item) {
                if (item.name == this.presetName) {
                    data = Finnova.Utils.clone(item.data || {});
                    return;
                }
            }, this);
            return data;
        ...
        loadSearchData: function () {
            ...
            if (this.presetName) {
                this.advanced = _.extend(Finnova.Utils.clone(this.getPresetData()), searchData.advanced);
                if (!primaryIsSet) {
                    this.primary = this.getPrimaryFilterName();
                }
            } else {
                this.advanced = Finnova.Utils.clone(searchData.advanced);
            }
