...
namespace Finnova\Entities;

class Email extends \Finnova\Core\ORM\Entity
{
    ...
    protected function _getFromName()
    {
        ...
        return \Finnova\Services\Email::parseFromName($this->get('fromString'));
    }

    protected function _getFromAddress()
    {
        if (!$this->has('fromString')) return null;
        return \Finnova\Services\Email::parseFromAddress($this->get('fromString'));
    }
    ...
    protected function _getReplyToName()
    {
        ...
        return \Finnova\Services\Email::parseFromName($string);
    }
    ...
    protected function _getReplyToAddress()
    {
        ...
        if (!$string) return null;
        $string = trim(explode(';', $string)[0]);
        return \Finnova\Services\Email::parseFromAddress($string);
    }
    ...
    public function addAttachment(\Finnova\Entities\Attachment $attachment)
    {
        if (!empty($this->id)) {
    ...
    {
        $this->set('messageId', 'dummy:' . \Finnova\Core\Utils\Util::generateId());
    }
}
