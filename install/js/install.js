...
InstallScript.prototype.setConnSett = function() {
        this.connSett.domainName = $('[name="domain-name"]').val();
...
InstallScript.prototype.validate = function() {
	...
	switch (this.action) {
		case 'step2':
			fieldRequired = ['domain-name','db-name', 'host-name', 'db-user-name', 'db-driver'];
			break;
		case 'step3':
...
InstallScript.prototype.getFinnovaPath = function(onlyPath) {
...
InstallScript.prototype.getModRewriteErrorMesssage = function() {
	...
	if (typeof(this.langs) !== 'undefined') {
		...
		message = message.replace("{ESPO_PATH}", this.getFinnovaPath(true)).replace("{API_PATH}", this.apiPath).replace("{API_PATH}", this.apiPath);
	}

	return message;
}

InstallScript.prototype.goToFinnova = function() {

	var location = this.getFinnovaPath();
