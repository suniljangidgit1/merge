...
class CronManager
{
    ...
    public function __construct(\Finnova\Core\Container $container)
    {
        ...
        $this->cronJobUtil = new \Finnova\Core\Utils\Cron\Job($this->config, $this->entityManager);
        $this->cronScheduledJobUtil = new \Finnova\Core\Utils\Cron\ScheduledJob($this->config, $this->entityManager);
	...
	$customDataPath = str_replace('{*}', $cacheDataPath, $this->lastRunTime);

        $lastRunData = $this->getFileManager()->getPhpContents($customDataPath);
	...
	$customDataPath = str_replace('{*}', $cacheDataPath, $this->lastRunTime);
        ...
        return $this->getFileManager()->putPhpContents($customDataPath, $data);
    }
    ...
    public function processPendingJobs($queue = null, $limit = null, $poolDisabled = false, $noLock = false)
    {
        ...
        foreach ($pendingJobList as $job) {
            ...
            if ($useProcessPool) {
                ...
                $job->set('pid', \Finnova\Core\Utils\System::getPid());
            }
            ...
            if ($useProcessPool) {
                $task = new \Finnova\Core\Utils\Cron\JobTask($job->id);
                $pool->add($task);
            } else {
                $this->runJob($job);
            }
    ...
    public function runJob(\Finnova\Entities\Job $job)
    {
        $isSuccess = true;
        $skipLog = false;
