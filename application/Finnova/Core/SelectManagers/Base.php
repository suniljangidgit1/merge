...
namespace Finnova\Core\SelectManagers;

use \Finnova\Core\Exceptions\Error;
use \Finnova\Core\Exceptions\Forbidden;

use \Finnova\Core\Acl;
use \Finnova\Core\AclManager;
...
class Base
{
    ...
    public function __construct(EntityManager $entityManager, \Finnova\Entities\User $user, Acl $acl, AclManager $aclManager, Metadata $metadata, Config $config, FieldManagerUtil $fieldManagerUtil, InjectableFactory $injectableFactory)
    {
    ...
    protected function getUser() : \Finnova\Entities\User
    {
        return $this->user;
    }
    ...
    protected function getSeed() : \Finnova\ORM\Entity
    {
        if (empty($this->seed)) {
    ...
    public function checkWhere(array $where, bool $checkWherePermission = true, bool $forbidComplexExpressions = false)
    {
        foreach ($where as $w) {
            ...
            if ($attribute && $checkWherePermission) {
                $argumentList = \Finnova\ORM\DB\Query\Base::getAllAttributesFromComplexExpression($attribute);
    ...
    protected function applyLeftJoinsFromAttribute(string $attribute, array &$result)
    {
        ...
        if (strpos($attribute, '.') !== false) {
            ...
            if ($this->getSeed()->hasRelation($link) && !$this->hasLeftJoin($link, $result)) {
                $this->addLeftJoin($link, $result);
                if ($this->getSeed()->getRelationType($link) === \Finnova\ORM\Entity::HAS_MANY) {
                    $result['distinct'] = true;
                }
            }
            return;
