...
define('views/record/base', ['view', 'view-record-helper', 'dynamic-logic'], function (Dep, ViewRecordHelper, DynamicLogic) {
    ...
    return Dep.extend({
        ...
        initDynamicLogic: function () {
            this.dynamicLogicDefs = Finnova.Utils.clone(this.dynamicLogicDefs || {});
            this.dynamicLogicDefs.fields = Finnova.Utils.clone(this.dynamicLogicDefs.fields);
            this.dynamicLogicDefs.panels = Finnova.Utils.clone(this.dynamicLogicDefs.panels);
        ...
        save: function (callback, skipExit, errorCallback) {
            ...
            data = _.extend(Finnova.Utils.cloneDeep(beforeSaveAttributes), data);
            ...
            // Script starts for to check GST fields modified or not
            var afterhash=window.location.hash;
            afterhash.indexOf(0);
            afterhash.toLowerCase();
            afterhash = afterhash.split("/")[0];
                    ...
                    {   
                        $.ajax({
                            ...
                            success: function(data){
                                if(data)
                                {
                                    // Code for edit form on click of name from account list
            ...
            // Script ends for to check GST fields modified or not
            ...
            model.save(setAttributes, {
                ...
                error: function (e, xhr) {
                    ...
                    if (response && response.reason) {
                        var methodName = 'errorHandler' + Finnova.Utils.upperCaseFirst(response.reason.toString());
        ...
        _doDependencyAction: function (data) {
            ...
            var methodName = 'dependencyAction' + Finnova.Utils.upperCaseFirst(action);
