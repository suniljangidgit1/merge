...
define('views/main', 'view', function (Dep) {
    ...
    return Dep.extend({
        ...
        init: function () {
            ...
            this.menu = Finnova.Utils.cloneDeep(this.menu);
            ...
            if (this.menu) {
                ['buttons', 'actions', 'dropdown'].forEach(function (type) {
                    (this.menu[type] || []).forEach(function (item) {
                        item = Finnova.Utils.clone(item);
                        menu[type] = menu[type] || [];

                        if (!Finnova.Utils.checkActionAvailability(this.getHelper(), item)) return;
                        if (!Finnova.Utils.checkActionAccess(this.getAcl(), this.model || this.scope, item)) return;
