...
class Application
{
    ...
    {
        return new \Finnova\Core\Utils\Auth($this->container);
    }
    ...
    public function runEntryPoint($entryPoint, $data = [], $final = false)
    {
        ...
        $entryPointManager = new \Finnova\Core\EntryPointManager($container);
        ...
        try {
            ...
            if ($authRequired && !$authNotStrict) {
                if (!$final && $portalId = $this->detectedPortalId()) {
                    $app = new \Finnova\Core\Portal\Application($portalId);
                    $app->setBasePath($this->getBasePath());
                    $app->runEntryPoint($entryPoint, $data, true);
                    exit;
                }
            }
            $auth = new \Finnova\Core\Utils\Auth($this->container, $authNotStrict);
            $apiAuth = new \Finnova\Core\Utils\Api\Auth($auth, $authRequired, true);
            $slim->add($apiAuth);
    ...
    public function runCron()
    {
        ...
        $cronManager = new \Finnova\Core\CronManager($this->container);
        $cronManager->run();
    }
    ...
    {
        return new \Finnova\Core\Utils\Api\Auth($auth);
    }
    ...
    {
        $routes = new \Finnova\Core\Utils\Route($this->getConfig(), $this->getMetadata(), $this->getContainer()->get('fileManager'));
    ...
    {
        $autoload = new \Finnova\Core\Utils\Autoload($this->getConfig(), $this->getMetadata(), $this->getContainer()->get('fileManager'));
        $autoload->register();
    }
