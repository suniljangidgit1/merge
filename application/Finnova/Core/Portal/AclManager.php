...
class AclManager extends \Finnova\Core\AclManager
{
    protected $tableClassName = '\\Finnova\\Core\\AclPortal\\Table';

    private $mainManager = null;

    private $portal = null;

    protected $userAclClassName = '\\Finnova\\Core\\Portal\\Acl';
    ...
    public function getImplementation($scope)
    {
        if (empty($this->implementationHashMap[$scope])) {
            ...
            $className = '\\Finnova\\Custom\\AclPortal\\' . $normalizedName;
            if (!class_exists($className)) {
                $moduleName = $this->getMetadata()->getScopeModuleName($scope);
                if ($moduleName) {
                    $className = '\\Finnova\\Modules\\' . $moduleName . '\\AclPortal\\' . $normalizedName;
                } else {
                    $className = '\\Finnova\\AclPortal\\' . $normalizedName;
                }
                if (!class_exists($className)) {
                    $className = '\\Finnova\\Core\\AclPortal\\Base';
                }
            }
