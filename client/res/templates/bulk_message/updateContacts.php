...
$crmEntityName     =   $_POST['crmEntityNameEdit'];
...
if(empty($upload_file) && empty($copy_paste) && empty($individual) && empty($crmEntityName) && empty($user_email))
{
    $data["error"]  = "true";
    $data["status"] = "false";
    $data["msg"]    = "Oops! Please select/upload contacts.";
...
// add induvidual || isset($user_email)
...
if($crmEntityName)
{
    ...
    if(!$result){
        $data["error"]  = "true";
        $data["status"] = "false";
        $data["msg"]    = "Oops! Contacts Duplicats";
        echo json_encode($data);return;
    }

    $sql = "SELECT totalcontacts, total_emails from contact_list WHERE id = '$list_id'";
    $res = mysqli_query($conn,$sql);
    $row=mysqli_fetch_array($res);

    $oldTotalContacts       =   $row['totalcontacts'];
    $newTotalContacts       =   $totalPhones + $oldTotalContacts;

    $oldTotalEmails         =   $row['total_emails'];
    $newTotalEmails         =   $totalEmails + $oldTotalEmails;

    $sql="UPDATE contact_list SET totalcontacts='$newTotalContacts', total_emails='$newTotalEmails' WHERE id = '$list_id'";
    $result=mysqli_query($conn,$sql);
