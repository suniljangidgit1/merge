...
/*$sql1="select * from invoice_files where invoice_id='$id'";
$result1=mysqli_query($conn,$sql1);
while($row1=mysqli_fetch_assoc($result1))
{		
	$link="../../client/res/templates/financial_changes/download_invoice_attachments.php?id=".$row1['id'];

	    $attachment_arr[] = array("id" => $row1['id'], "invoice_id" => $row1['invoice_id'], "filepath" => $row1['filepath'], "original_filename" => $row1['original_filename'],"link" => $link);
	...
	$res = $zip->open($file_path);
  	if($res == TRUE)
  	{
		...
		for($i = 0; $i < $zip->numFiles; $i++)
		{
			...
			$attachment_arr[] = array("id" => $row1['id'], "invoice_id" => $id, "filepath" => $file_path, "original_filename" => $ExtractFileName,"link" => $link);
      	}
      	$zip->close();
  	}
}
