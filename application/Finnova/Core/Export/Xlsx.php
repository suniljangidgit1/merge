...
namespace Finnova\Core\Export;

use \Finnova\ORM\Entity;
use \Finnova\Core\Exceptions\Error;

class Xlsx extends \Finnova\Core\Injectable
{
	...
	include('application/Finnova/Core/Utils/myconfig.php');
        if (!$this->getInjection('fileManager')->isDir('data/cache/'.$cacheDataPath.'/')) {
            $this->getInjection('fileManager')->mkdir('data/cache/'.$cacheDataPath.'/');
        }
        $tempFileName = 'data/cache/'.$cacheDataPath.'/' . 'export_' . substr(md5(rand()), 0, 7);
