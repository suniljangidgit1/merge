    ...
    return Dep.extend({
        ...
        setup: function () {
            ...
            this.attributePartList.forEach(function (part) {
                this.allAddressAttributeList.push(this.copyFrom + Finnova.Utils.upperCaseFirst(part));
                this.allAddressAttributeList.push(this.name + Finnova.Utils.upperCaseFirst(part));
            }, this);
        ...
        copy: function (fieldFrom) {
            var attrList = Object.keys(this.getMetadata().get('fields.address.fields')).forEach(function (attr) {
                destField = this.name + Finnova.Utils.upperCaseFirst(attr);
                sourceField = fieldFrom + Finnova.Utils.upperCaseFirst(attr);
        ...
        toShowCopyButton: function () {
            ...
            this.attributePartList.forEach(function (part) {
                var attribute = this.copyFrom + Finnova.Utils.upperCaseFirst(part);
                if (this.model.get(attribute)) {
                    billingIsNotEmpty = true;
                }
                var attribute = this.name + Finnova.Utils.upperCaseFirst(part);
                if (this.model.get(attribute)) {
