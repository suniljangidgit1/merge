...
$userId		=	$row['id'];
 ...
 function unlinkFile(){
		...
		// $data["status"] = "true";
		// $data["error"] 	= "false";
		// $data["msg"] 	= "Opps! something wents wrong.";

		$result = emailRminderGet($er_id);
		
		if( !empty($er_id) && !empty($er_fileName) && !empty($result) ){

			if( !empty($result["file_name"]) ){

				$filePath = "../reminder/zipFolder/".$result["file_name"];
		...
		}else if (empty($er_id) && !empty($er_fileName)) {
			$target_dir = '../reminder/uploads/'.$er_fileName;
	...
	function emailRminderGet($id=""){
	
	 	GLOBAL $conn;
		$sql1 = 'SELECT er.file_name FROM email_reminder as er WHERE id="'.$id.'" ORDER BY id ASC ';
