...
namespace Finnova\Core\Mail;

use \Zend\Mime\Mime as Mime;

use \Finnova\ORM\Entity;
use \Finnova\ORM\Email;
...
class Importer
{
    ...
    public function importMessage($parserType = 'ZendMail', $message, $assignedUserId = null, $teamsIdList = [], $userIdList = [], $filterList = [], $fetchOnlyHeader = false, $folderData = null)
    {
        ...
        $parserClassName = '\\Finnova\\Core\\Mail\\Parsers\\' . $parserType;
        ...
        if ($parser->checkMessageAttribute($message, 'message-Id') && $parser->getMessageAttribute($message, 'message-Id')) {
            ...
            if (stripos($messageId, '@finnova-system') !== false) {
                return;
            }
        }
        ...
        if ($parser->checkMessageAttribute($message, 'references') && $parser->getMessageAttribute($message, 'references')) {
            ...
            $number = null;
            $n = sscanf($reference, '%s %s %d %d finnova', $parentType, $parentId, $emailSent, $number);
            if ($n != 4) {
                $n = sscanf($reference, '%s %s %d %d finnova-system', $parentType, $parentId, $emailSent, $number);
            }
