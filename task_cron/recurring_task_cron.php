<?php
	...
	echo $subArr[0]."<br><br><br>";


	$conn = mysqli_connect('finnovadb.cg6cxmb6q5aw.ap-south-1.rds.amazonaws.com', 'admin', '7vG&^Z%$fvnH', 'crmdev');

	$recurring_task_master_id = "";

	if($conn->connect_error){
		die("Connection failed ". $conn->connect_error);
	}

	$res = mysqli_query($conn, "SELECT * FROM `recurring_task_master`");

	while( $row = mysqli_fetch_array($res) ){
		$recurring_task_master_id= $row['id'];
		// echo "TASK ID = ". $row['task_id']."<br>";
		// echo "SUBDOMAIN = ". $row['subdomain']."<br>";
		// echo "CREATED AT = ". $row['created_at']."<br><br><br><br><br>";

		$conn = mysqli_connect('10.0.2.245', 'root', 'rZdfvsHax*4', $row['subdomain']);
		if($conn->connect_error){
			die("Connection failed ". $conn->connect_error);
		}
