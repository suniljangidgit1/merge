...
class Upgrade extends Base
{
    ...
    protected function runLocalUpgrade(array $params)
    {
        ...
        if (!$params['skipConfirmation']) {
            fwrite(\STDOUT, "FinnovaCRM will be upgraded to version {$nextVersion} now. Enter [Y] to continue.\n");
    ...
    protected function getUpgradeManager($reload = false)
    {
        if (!$this->upgradeManager || $reload) {
            ...
            $this->upgradeManager = new \Finnova\Core\UpgradeManager($app->getContainer());
        }
    ...
    {
        $url = 'https://s.fincrm.net/upgrade/next/';
    ...
    {
        $localFilePath = 'data/upload/upgrades/' . \Finnova\Core\Utils\Util::generateId() . '.zip';
