...
namespace Finnova\Core\Utils\Database\Orm;
...
class Converter
{
    ...
    */
    ...
    public function __construct(\Finnova\Core\Utils\Metadata $metadata, \Finnova\Core\Utils\File\Manager $fileManager, \Finnova\Core\Utils\Config $config = null)
    {
        ...
        $this->databaseHelper = new \Finnova\Core\Utils\Database\Helper($this->config);
    }
            ...
            foreach ($entityParams['fields'] as $fieldName => &$fieldParams) {
                ...
                switch ($fieldParams['type']) {
                    ...
                    default:
                        ...
                        if (!defined('\\Finnova\\ORM\\Entity::' . $constName)) {
    ...
    /**
     * Metadata conversion from Finnova format into Doctrine
     *
    ...
    /**
     * Correct fields definitions based on \Finnova\Custom\Core\Utils\Database\Orm\Fields
     *
    ...
    protected function correctFields($entityName, array $ormMetadata)
    {
        ...
        foreach ($entityMetadata['fields'] as $fieldName => $fieldParams) {
            ...
            $className = '\Finnova\Custom\Core\Utils\Database\Orm\Fields\\' . $fieldType;
            if (!class_exists($className)) {
                $className = '\Finnova\Core\Utils\Database\Orm\Fields\\' . $fieldType;
            }
    ...
    {
        $values = array();
        foreach($this->fieldAccordances as $finnovaType => $ormType) {

            if (isset($fieldParams[$finnovaType])) {

                if (is_array($ormType))  {

                    $conditionRes = false;
                    if (!is_array($fieldParams[$finnovaType])) {
                        $conditionRes = preg_match('/'.$ormType['condition'].'/i', $fieldParams[$finnovaType]);
                    }

                    if (!$conditionRes || ($conditionRes && $conditionRes === $ormType['conditionEquals']) )  {
                        $value = is_array($fieldParams[$finnovaType]) ? json_encode($fieldParams[$finnovaType]) : $fieldParams[$finnovaType];
                        $values = Util::merge( $values, Util::replaceInArray('{0}', $value, $ormType['value']) );
                    }
                } else {
                    $values[$ormType] = $fieldParams[$finnovaType];
                }
