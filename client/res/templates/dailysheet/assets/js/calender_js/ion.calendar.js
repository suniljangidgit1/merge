        ...
        init: function(options){
            ...
            return this.each(function(){
                ...
                var prepareCalendar = function(){
                    ...
                    if(parseInt(timeNowLocal.format("YYYY")) == currentYear){
                ...
                }else if(parseInt(timeNowLocal.format("YYYY")) < currentYear){
                    for(i = 0; i < 12; i++){
                        if(i === parseInt(timeNowLocal.format("M")) - 1){
                            html += '<option value="' + i + '" selected="selected">' + timeForWork.month(i).format("MMMM") + '</option>';
                        } else {
                    ...
                    html += '</div>';
                    html += '<div class="attendance-border-top"></div>';

                    if(settings.sundayFirst) {

                        // week
                        html += '<div class="table-responsive"><table class="ic__week-head"><tr>';
                        for(i = 0; i < 7; i++) {
                        ...
                        // month
                        html += '<div class="table-responsive"><table class="ic__days"><tr>';
                        // empty days
                        ...
                        for(i = 1; i <= monthLastDay; i++) {
                            ...
                            if(moment(timeNowLocal).date(i).format("D.M.YYYY") === timeNow.format("D.M.YYYY")) {
                                html += '<td class="ic__day ic__day_state_current" data-toggle="tooltip" title="In Time \nOut time\nWorking hour" >' + i + '</td>';
                            } else if(timeSelected && moment(timeNowLocal).date(i).format("D.M.YYYY") === timeSelected.format("D.M.YYYY")) {
                                html += '<td class="ic__day ic__day_state_selected" data-toggle="tooltip" title="In Time\nOut time\nWorking hour">' + i + '</td>';
                            } else {
                                html += '<td class="ic__day" data-toggle="tooltip" title="In Time\nOut time\nWorking hour" > ' + i + '</td>';
                            }
                ...
                var placeCalendar = function(){
                    ...
                    if(settings.hideArrows) {
                        ...
                        var currentYear = (new Date).getFullYear();
                            e.preventDefault();
                            if(parseInt(timeNowLocal.format("YYYY")) == currentYear){
                                if(parseInt(timeNowLocal.format("M")) < currentMonth ){
                                    timeNowLocal.add("months", 1);
                                }
                            }else if(parseInt(timeNowLocal.format("YYYY")) < currentYear){
                                timeNowLocal.add("months", 1);
