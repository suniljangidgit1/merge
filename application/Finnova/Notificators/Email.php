...
namespace Finnova\Notificators;

use \Finnova\ORM\Entity;

class Email extends \Finnova\Core\Notificators\Base
{
    ...
    public function process(Entity $entity, array $options = [])
    {
        ...
        if (empty($data['personEntityId'])) {
            $data['fromString'] = \Finnova\Services\Email::parseFromName($entity->get('fromString'));
